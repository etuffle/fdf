...
/*   Updated: 2019/03/03 18:15:37 by etuffleb         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	mlx_line_put(t_coords *coord, t_mlx mlx, int color, int tg)
{
	if (tg)
	{
		mlx_pixel_put(mlx.ptr, mlx.win, (int)coord->x, (int)coord->y, ft_opasity(color, coord->x, 1));
		mlx_pixel_put(mlx.ptr, mlx.win, (int)coord->x + 1, (int)coord->y, ft_opasity(color, coord->x, 0));
		if (tg == -1)
			(coord->y)++;
		else
			(coord->y)--;
	...
	{
		mlx_pixel_put(mlx.ptr, mlx.win, (int)coord->x, (int)coord->y, ft_opasity(color, coord->y, 1));
		mlx_pixel_put(mlx.ptr, mlx.win, (int)coord->x, (int)coord->y + 1, ft_opasity(color, coord->y, 0));
		(coord->x)++;
	}
}
...
void	put_nice_line(t_point one, t_point two, t_mlx mlx, t_gradient color)
{
	...
	if (fabs(two.x - one.x) < fabs(two.y - one.y))
		if (two.y < one.y)
			while (coord.y >= two.y)
			{
				mlx_line_put(&(coord), mlx, ft_gradient(one.y, two.y, color, coord.y), 1);
		...
		while (coord.x <= two.x)
		{
			mlx_line_put(&(coord), mlx, ft_gradient(one.x, two.x, color, coord.x), 0);
